...
def prepare_players_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Prepare player dataframe before visualization."""
    df = df.copy()
    ...
    df.loc[
        (df["player_height"] > 195) & (df["player_height"] < 207),
        "position"
    ] = "Forward"
...
def add_start_year(df: pd.DataFrame) -> pd.DataFrame:
    """Recreate start_year after groupby operations."""
    df = df.copy()
    df["season"] = df["season"].astype(str)
    df["start_year"] = df["season"].str[:4].astype(int)
    return df
...
def add_small_ball_marker(fig, y=None):
    """Add visual marker for the 2015-16 Warriors turning point."""
    fig.add_vline(
        x="2015-16",
        line_width=2,
        line_dash="dash",
...
def get_avg_height_by_season_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
    ...
    grouped = add_start_year(grouped)
...
def get_avg_weight_by_season_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_rebounds_vs_height_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_total_assists_by_season_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_center_assists_by_season_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_points_by_position_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_rebounds_by_position_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
...
def get_assists_by_position_figure(df: pd.DataFrame):
    df = prepare_players_dataframe(df)
